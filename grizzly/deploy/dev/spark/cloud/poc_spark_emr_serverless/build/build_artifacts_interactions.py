...
def _get_poetry_package_name_and_version(package_version: str = None) -> Tuple[str, str]:
    """
    ...
    """
    package_name, current_package_version = _get_current_poetry_package_name_and_version()
...
def get_poetry_wheel_file(
    file_folder: str = "deploy/dev/spark/cloud/poc_spark_emr_serverless/build/artifacts/package_wheel_files",
    ...
    """
    package_name, package_version = _get_poetry_package_name_and_version(package_version=package_version)
...
def rename_poetry_wheel_file(
    ...
    :return: None
    """
    package_name, package_version = _get_poetry_package_name_and_version(package_version=None)
    ...
    if len(current_package_wheel_file) == 0:
        ...
        os.rename(f"{base_dir}/{file_folder}/{wheel_file_name}", f"{base_dir}/{file_folder}/{new_wheel_file_name}")
    else:
        raise ValueError(
...
# ----------- Venv file functions -----------
def get_venv_file(
    file_folder: str = "deploy/dev/spark/cloud/poc_spark_emr_serverless/build/artifacts/venvs",
    package_version: str = None,
) -> str:
    """
    Get venv file name
...
def add_package_version_to_venv(
    ...
    :return: None
    """
    new_venv_file_name = get_venv_file(file_folder=file_folder, package_version=None)
    os.rename(f"{base_dir}/{file_folder}/{file_name}", f"{base_dir}/{file_folder}/{new_venv_file_name}")
