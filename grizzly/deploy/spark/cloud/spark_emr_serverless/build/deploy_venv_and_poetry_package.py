...
import grizzly.deploy.spark.cloud.spark_emr_serverless.build
from grizzly.iac_pulumi.pulumi_rest_api_functions import get_pulumi_stack_state
from grizzly.deploy.spark.cloud.spark_emr_serverless.get_config_variables import get_s3_bucket_id_from_pulumi
...
def deploy_venv_and_poetry_package(main_config: Dict[str, Any]):
    ...
    build_dir = os.path.dirname(
                inspect.getfile(grizzly.deploy.spark.cloud.spark_emr_serverless.build)
    )
